...
fn run_tui_on_main_thread_impl(
    ...
    match runtime::drive_session(&mut guard, &state, &rx, &controller, comp_handle)? {
        runtime::SessionOutcome::Complete => Ok(()),
        runtime::SessionOutcome::Abort => {
            drop(guard);
            std::process::exit(2);
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_keyboard_command_families_preserve_navigation_and_exit_precedence() {
        ...
        assert_eq!(
            runtime::handle_event(
                Event::Key(KeyEvent::from(KeyCode::PageDown)),
                &state,
                &controller
            ),
        ...
        assert_eq!(state.lock().unwrap().io_chart_mode, IoChartMode::Cumulative);
