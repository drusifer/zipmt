...
```
- **Setup once — `make judge-tools-install`**: creates the isolated `.judge-venv` and installs the pinned Tracegate normalizer. Do not install Judge dependencies into system Python.
- **Required tool — `make judge-trace [DATE=YYYY-MM-DD] [FORMAT=html|md] [SOURCE=auto|codex|claude]`**: this runs `agents/tools/trace_annotate.py`, discovers real Codex CLI rollouts (`$CODEX_HOME/sessions/YYYY/MM/DD/rollout-*.jsonl`) and Claude Code transcripts (`~/.claude/projects/<slug>/*.jsonl`), then delegates native trace parsing and normalization to **Tracegate**. Judge's thin adapter maps Tracegate events onto the existing anti-pattern rules in `agents/tools/trace_rules.json`. `SOURCE=auto` includes both available harnesses; use an explicit source while diagnosing ingestion. Output defaults to `agents/trin.docs/judge_tool_trace.{html,md}`.
...
- **Tool-call efficiency rubric**: Judge adopts MLflow `ToolCallEfficiency`'s core question—whether the trajectory contains redundant or unnecessary calls—but evaluates it locally and deterministically over Tracegate events. Review `AP-DUP-TOOL`, `AP-UNCHANGED-RETRY`, `AP-RETEST-NO-CHANGE`, `AP-SEARCH-LOOP`, and `AP-DUP-READ`. An efficiency verdict is **yes** only after every flagged call is either confirmed necessary with a concrete state/scope change or removed; otherwise it is **no**. Always identify the specific redundant calls and why they could be reused, changed, consolidated, or stopped.
